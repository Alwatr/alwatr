...
export class AlwatrPageOrderDetail extends LocalizeMixin(SignalMixin(UnresolvedMixin(AlwatrBaseElement))) {
  ...
  override render(): unknown {
    ...
    this._editMode = this.order?.status === 'draft';
    if (this.hasAttribute('edit-mode') !== this._editMode) {
      this.toggleAttribute('edit-mode', this._editMode);
    }
      ...
      <alwatr-surface elevated>
        <div class="detail-container">
          <div>
            ...
            <span>
              <b>${number(this.order.shippingPrice)}</b>
              ${this.tomanSvg}
            </span>
          </div>
          <div>
            ...
            <span>
              <b>${number(this.order.totalPrice)}</b>
              ${this.tomanSvg}
            </span>
          </div>
          <div>
            ...
            <span>
              <b>${number(this.order.finalPrice)}</b>
              ${this.tomanSvg}
            </span>
          </div>
        </div>
  ...
  protected _itemDetailTemplate(item: OrderItem): unknown {
    const product = this._productStorage?.data[item.productId];
    if (product == null) {
      ...
      <alwatr-surface elevated>
        ...
        <div class="detail-container">
          ...
          <div>
            ...
            ${this._editMode === true
              ? html`<alwatr-text-field type="number" .value=${item.qty.toString()} outlined></alwatr-text-field>`
              : html`<span><b>${item.qty}</b></span>`}
          </div>
        </div>
      </alwatr-surface>
    `;
