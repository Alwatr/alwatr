...
export class AlwatrPwaDirective extends AlwatrDynamicDirective {
  ...
  * render(): unknown {
    ...
    yield html`
      ...
      <!-- App container -->
      <div class="flex h-full max-h-full w-full max-w-full flex-col flex-nowrap items-stretch overflow-clip extended:ps-[22.5rem] medium:ps-20">
        <!-- Top app bar -->
        ${this._renderTopAppBar()}

        <!-- Pages container -->
        <main class="flex grow flex-wrap gap-8 p-8">
          ${this._renderContent()}
        </main>
  ...
  protected _renderTopAppBar(): unknown {
    ...
    <header
        ...
        >
          <button
            class="inline-block cursor-pointer rounded-full p-3 text-onSurface"
          ...
          <button class="inline-block cursor-pointer rounded-full p-3 text-onSurfaceVariant">
            <alwatr-icon name="calendar-number-outline" class="h-6 w-6"></alwatr-icon>
          </button>
        </header>
    `;
  }
