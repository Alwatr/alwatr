...
| `@alwatr/action`          | Action layer for Unidirectional Data Flow. HTML attribute `on-action="eventType[.modifier…]->actionId[:payload]"` bridges DOM events to typed signal handlers via **global event delegation** (O(1) boot). Subscribe: `onAction(actionId, handler)`. Dispatch: `dispatchAction(actionId, payload)`. Bootstrap: `setupActionDelegation()`. |
