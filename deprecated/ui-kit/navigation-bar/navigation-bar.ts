...
export class AlwatrNavigationBarDirective extends AlwatrDirective {
  ...
  render(content?: NavigationBarContent): unknown {
    ...
    return html` <footer class="shrink-0 grow-0 bg-surfaceContainer elevation-2">
      <nav
  ...
  protected _renderNavItems(content: NavigationBarContent): unknown {
    const navItemList = mapObject(content.itemList, (item, key) => {
      ...
      return html`<a
        aria-selected=${content.selected === key}
        class="group flex grow flex-col items-center justify-start pt-3
        hover:text-onSurface aria-selected:pointer-events-none"
        ><div
          class="rounded-2xl px-5 py-1 group-hover:stateHover-onSurfaceVariant group-active:stateActive-onSurfaceVariant group-aria-selected:bg-secondaryContainer
        group-aria-selected:text-onSecondaryContainer [&>.alwatr-icon]:block
        [&>.alwatr-icon]:h-6 [&>.alwatr-icon]:w-6"
        >
          ${icon(item.icon)}
        </div>
        ${when(_label, () => html`<div class="py-1 group-aria-selected:text-onSurface">${_label}</div>`)}</a
      >`;
    });
