...
document.getElementById('requestButton')?.addEventListener('click', () => {
  ...
  commandAddTrigger.request({a: 10, b: 20});
  console.info('2. returned');
});

document.getElementById('requestButton2')?.addEventListener('click', () => {
  console.info('1. request command (10+20)');
  const value = commandAddTrigger.request({a: 10, b: 20});
  ...
  void Promise.all([value1p, value2p]).then((values) => {
    console.info('4. returned (%s)', values.join(', '));
  });
});
